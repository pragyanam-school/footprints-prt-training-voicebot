...
Reason: ${e4}`);
  ...
  function formatDate(dateStr) {
    return (/* @__PURE__ */ new Date(dateStr + "Z")).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });
  }
  ...
  function getDateRange(filter) {
    ...
    if (filter === "today") {
      ...
      dateTo = new Date(today.getTime() + 864e5).toISOString();
    } else if (filter === "yesterday") {
      dateFrom = new Date(today.getTime() - 864e5).toISOString();
      dateTo = today.toISOString();
    } else if (filter === "7days") {
      dateFrom = new Date(today.getTime() - 7 * 864e5).toISOString();
      dateTo = new Date(today.getTime() + 864e5).toISOString();
      ...
      dateTo = document.getElementById("customTo")?.value ? (/* @__PURE__ */ new Date(document.getElementById("customTo").value + "T23:59:59")).toISOString() : null;
    }
  ...
  function showCallActive() {
    ...
    status.innerHTML = `<strong>\u{1F3AF} ${currentPersona}</strong><br><span style="font-size:13px;color:#2e7d32">Call in progress. Handle the enquiry naturally.</span>`;
  }
  ...
  async function loadSummary(agentId) {
    ...
    <div class="summary-grid">
      ...
      <div class="summary-stat">
        <div class="summary-number">\u{1F525} ${s.streak}</div>
  ...
  ${score.parameters ? score.parameters.map((p) => {
    ...
    if (currentUser.role === "supervisor") {
      ...
      <div style="margin-top:24px">
        ...
        ${notes.map((n) => `
          <div class="note-item">
            <div style="font-size:12px;color:#636e72;margin-bottom:4px">${n.supervisor_name} \xB7 ${formatDate(n.created_at)}</div>
